...
static char *getFormatedpwd(char *newPwd, char *oldpwd, char *home)
{
  char  *pwd;
  int   i;
  ...
  while (newPwd[i])
  {
    if (newPwd[i] == '.' && newPwd[i + 1] != '\0'\
      && newPwd[i + 1] == '.' && newPwd[i + 2] != '\0' && newPwd[i + 2] == '.')
    {
      pwd = del_path(pwd, 2);
      i += 2;
    }
    else if (newPwd[i] == '.' && newPwd[i + 1] != '\0' && newPwd[i + 1] == '.')
    {
      pwd = del_path(pwd, 1);
      i += 1;
    }
    else if (newPwd[i] == '~' && i == 0)
      pwd = ft_dstrjoin(pwd, home, 1);
    else if (newPwd[i] == '-' && i == 0)
      pwd = ft_dstrjoin(pwd, oldpwd, 1);
...
static BOOLEAN ft_cd(t_client *client, char *newPwd)
{
  ...
  pwd = getFormatedpwd(newPwd, client->oldpwd, client->home);
  if (!is_dir(pwd))
  {
    ft_strdel(&newPwd);
    ft_strdel(&pwd);
